...
TRPC_RE = re.compile(
    r"""\b(?:trpc|api)\.((?:[A-Za-z_]\w*\.)+)use(?:Query|Mutation|InfiniteQuery|SuspenseQuery)\b"""
)
TS_REST_PATH_RE = re.compile(r"""path\s*:\s*['"]([^'"]+)['"]""")
...
def _looks_like_graphql_codegen(rel: str, source: str) -> bool:
    ...
    return any(
        tok in blob
        for tok in ("__typename", "typeddocumentnode", "graphql-codegen", "/generated/", "gql.ts", "graphql.ts")
    )
...
def _extract_next_routes(add, edge, graph, rel, source, feature, app_info, pages_info, components) -> None:
    ...
    if info.get("api"):
        _add_client(
            ...
            [],
            components,
            route if route.startswith("/api/") else "/api" + (route if route.startswith("/") else "/" + route),
            line,
...
def _extract_typed_clients(add, edge, graph, rel, source, feature, hooks, components) -> None:
    if RTK_RE.search(source) or "injectEndpoints" in source:
        ...
        for m in RTK_ENDPOINT_RE.finditer(source):
            ...
            raw = ""
            if url_m:
                raw = url_m.group(1) or url_m.group(2) or url_m.group(3) or ""
            if not raw or raw.startswith("http") and "/api/" not in raw and not raw.startswith("/"):
                # still record the endpoint name as a typed client when we have a path-like string later
                pass
            if raw and (raw.startswith("/") or "/api/" in raw or "{" in raw or ":" in raw):
    ...
    if "createClient" in source or ".GET(" in source or ".POST(" in source:
    ...
    if "initContract" in source or "@ts-rest" in source or "ts-rest" in rel:
        for m in TS_REST_PATH_RE.finditer(source):
            raw = m.group(1)
            if not raw.startswith("/") and "/api/" not in raw:
                continue
            line = source[: m.start()].count("\n") + 1
...
def _extract_e2e(add, edge, rel, source, feature, stem) -> None:
    ...
    for tmpl in visits:
        ...
        if "/api/" in tmpl:
            edge(
                node_id(NodeType.API_CLIENT, f"client:{tmpl}"),
                tn.id,
                EdgeType.TESTED_BY,
                extra={"via": "e2e"},
                confidence=0.5,
